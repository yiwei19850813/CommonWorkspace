...
@property (nonatomic)BOOL onlyShowIntNum;
@end
...
- (void)setPopUpViewHiden:(BOOL)b
{
    self.popUpView.hidden = b;
}

//设置popupView只显示整数
- (void)setPopUpViewOnlyShowIntNum:(BOOL)onlyShowIntNum   //add by xdyang
{
    self.onlyShowIntNum = onlyShowIntNum;
}
...
- (void)setup
{
    [self setThumbImage:[UIImage imageNamed:@"btn_sppd"] forState:UIControlStateNormal];
...
- (void)positionAndUpdatePopUpView
{
    ...
    NSString *numStr = [_numberFormatter stringFromNumber:@(self.value)];
    if(self.onlyShowIntNum){
        CGFloat num = [numStr floatValue];
        int numInt = num;
        numStr = @(numInt).stringValue;
    }
    [self.popUpView setString:numStr];
...
- (void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    [self positionAndUpdatePopUpView];
    if (self.delegate && [self.delegate respondsToSelector:@selector(onEndTrack:)])
    {
        [self.delegate onEndTrack:self];
    }
    [self.popUpView hide];
}
